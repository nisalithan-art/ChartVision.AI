...
st.markdown("""
    ...
    /* Premium Data Cards / Analytics Container */
    .analysis-card {
        ...
        try:
            ...
            if data.empty:
                ...
                if 'Close' in data.columns:
                    close_series = pd.Series(data['Close'].values.flatten(), index=data.index).dropna()
                    high_series = pd.Series(data['High'].values.flatten(), index=data.index).dropna()
                    low_series = pd.Series(data['Low'].values.flatten(), index=data.index).dropna()
                    open_series = pd.Series(data['Open'].values.flatten(), index=data.index).dropna()
                    ...
                    if len(close_series) < 30:
                        ...
                        fig.add_trace(go.Candlestick(
                            x=data.index, open=open_series, high=high_series, low=low_series, close=close_series, name=ticker
                        ))

                        fig.add_trace(go.Scatter(x=data.index, y=upper_bb, line=dict(color='rgba(0, 255, 204, 0.25)', width=1.5, dash='dash'), name='Upper Bollinger Band'))
                        fig.add_trace(go.Scatter(x=data.index, y=lower_bb, line=dict(color='rgba(0, 255, 204, 0.25)', width=1.5, dash='dash'), name='Lower Bollinger Band'))
                        ...
                        with col4:
                            bb_pct = ((current_price - current_lower_bb) / (current_upper_bb - current_lower_bb)) * 100
